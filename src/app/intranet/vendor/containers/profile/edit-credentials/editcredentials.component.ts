...
export class EditcredentialsComponent implements OnInit {
    ...
    vendorCredentialsForm: FormGroup | any;

    ngOnInit() {
        this.vendorCredentialsForm = this.fb.group ({
    ...
    updateCredentialsVendor(){
        this.userService.updateCredentialsVendor(this.vendorCredentialsForm.value).subscribe(result =>{
            if(!result['update']){
                Swal.fire({
